...
{
  // Handle 'b target' pseudo-instruction: expands to BEQ $zero,$zero,wordOffset.
  // If target > 0xFFFF it is treated as an absolute N64 virtual address and
  // converted to a PC-relative word offset (target - (pc+4)) / 4.
  if (Operands.size() == 2) {
    MipsOperand &MnemOp = static_cast<MipsOperand&>(*Operands[0]);
    if (MnemOp.isToken() && MnemOp.getToken().lower() == "b") {
      MipsOperand &TargetOp = static_cast<MipsOperand&>(*Operands[1]);
      if (TargetOp.isConstantImm()) {
        int64_t target  = TargetOp.getConstantImm();
        int64_t wordOff = ((uint64_t)target > 0xFFFFULL)
                          ? (target - ((int64_t)Address + 4)) / 4
                          : target;
        // Build BEQ $0,$0,wordOff directly.
        // BEQ encoding: opcode(4)=6b|rs(5)|rt(5)|offset(16)
        // = 000100 00000 00000 <wordOff:16>
        // We write raw bytes to bypass MCCodeEmitter fixups, which would
        // incorrectly apply fixup_Mips_PC16 on top of our already-computed
        // word offset (causing double-transformation).
        uint32_t encoding = (4u << 26) | ((uint32_t)(wordOff & 0xFFFF));
        bool emitError = false;
        Out.EmitIntValue(encoding, 4, emitError);
        ErrorCode = emitError ? KS_ERR_ASM_MIPS_INVALIDOPERAND : 0;
        if (ErrorCode == 0) {
          Address += 4;
          return false;
        }
                                     ...
                                     SMLoc NameLoc, OperandVector &Operands, unsigned int &ErrorCode)
{
  ...
  // 'b target' pseudo-instruction — unconditional branch.
  // Handled entirely in MatchAndEmitInstruction where Address is available.
  if (Name.lower() == "b") {
